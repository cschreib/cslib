; docformat='IDL'
pro stsci_density, x0, y0, drange=drange, dlog=dlog, dsize=dsize, $
...
if (!d.name eq 'PS') then begin
  ...
  g = g0
  b = b0
  r[1] = keyword_set(ps_white) ? 255 : r[0]
  g[1] = keyword_set(ps_white) ? 255 : g[0]
  b[1] = keyword_set(ps_white) ? 255 : b[0]
  tvlct,r,g,b
endif

; initial plot of axes to define the plot region.
idf = where(finite(x) and finite(y))
minx = min(x[idf],max=maxx)
miny = min(y[idf],max=maxy)
