...
;
function bin_map, x, y, z, numbins=numbins, xr=xr, yr=yr, xdata=xdata, ydata=ydata, median=median
    ...
    for xi=0L, numbins[0]-1 do begin
        ...
        for yi=0L, numbins[1]-1 do begin
            ...
            if cnt ne 0 then begin
                if keyword_set(median) then val = median(z[tmp1[tmp2]]) else val = mean(z[tmp1[tmp2]])
            endif else begin
                val = !values.f_nan
            endelse
