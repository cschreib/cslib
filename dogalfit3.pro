function dogalfit3_read_param, hdr, name
...
pro dogalfit3, filename, imgsize, output=output, error=error, psf=psf, badpix=badpix, aspix=aspix, $
    ...
    setindex=setindex, double_sersic=double_sersic, model=model, hdr=hdr, cleanup=cleanup, $
    showfit=showfit, tmp_dir=tmp_dir, samepos=samepos, max_dpos=max_dpos, sameangle=sameangle, $
    nobg=nobg, setbt=setbt, params=params
    ...
    if extpos lt 0 then begin
        print, 'error: input file is not a FITS image'
        success = 0
        return
    endif
    ...
    if ~provided(magzp) then magzp = 25.0
    ...
    if keyword_set(double_sersic) then begin
        params = { $
            ...
            radius1:!values.f_nan, radius2:!values.f_nan, angle1:!values.f_nan, $
            axis_ratio1:!values.f_nan, axis_ratio2:!values.f_nan, $
            angle2:!values.f_nan, flux1:!values.f_nan, flux2:!values.f_nan, $
            index1:!values.f_nan, index2:!values.f_nan, chi2:!values.f_nan $
        }

        if success then begin
            pos1 = [dogalfit3_read_param(hdr, '1_XC'), $
                    dogalfit3_read_param(hdr, '1_YC')]
            pos2 = [dogalfit3_read_param(hdr, '2_XC'), $
                    dogalfit3_read_param(hdr, '2_YC')]
            ...
            params.radius1     = dogalfit3_read_param(hdr, '1_RE')
            params.axis_ratio1 = dogalfit3_read_param(hdr, '1_AR')
            params.angle1      = dogalfit3_read_param(hdr, '1_PA')
            params.index1      = dogalfit3_read_param(hdr, '1_N')
            tmag1           = dogalfit3_read_param(hdr, '1_MAG')
            params.flux1 = 10d0^(-0.4*tmag1)

            params.radius2     = dogalfit3_read_param(hdr, '2_RE')
            params.axis_ratio2 = dogalfit3_read_param(hdr, '2_AR')
            params.angle2      = dogalfit3_read_param(hdr, '2_PA')
            params.index2      = dogalfit3_read_param(hdr, '2_N')
            tmag2              = dogalfit3_read_param(hdr, '2_MAG')
            params.flux2 = 10d0^(-0.4*tmag2)

            params.chi2 = dogalfit3_read_param(hdr, 'CHI2NU')
        endif
    endif else begin
        params = { $
        ...
        if success then begin
            ...
            params.radius     = dogalfit3_read_param(hdr, '1_RE')
            params.axis_ratio = dogalfit3_read_param(hdr, '1_AR')
            params.angle      = dogalfit3_read_param(hdr, '1_PA')
            params.index      = dogalfit3_read_param(hdr, '1_N')
            tmag1             = dogalfit3_read_param(hdr, '1_MAG')
            params.flux = 10d0^(-0.4*tmag1)
