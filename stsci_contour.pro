; docformat='IDL'
pro stsci_contour, x0, y0, drange=drange, dlog=dlog, dsize=dsize, $
  color=color, thick=thick, flevels=flevels, $
  xlog=xlog, ylog=ylog, overplot=overplot, _extra = _extra
;+
...
; axes are drawn with color = 255 (~ 0).
;
...
; is done. By default, the values of the contour LEVELS are ~ especially
...
; (~ enclose) the designated fraction of the data. You'll need to use
; the C_ANNOTATION parameter to label the contours appropriately.
; Note: the contours may ~ be precise in some cases (because of smoothing)
...
if (dsize[1] gt 100) then begin
  kern = shift(exp(-0.5*dist(20)^2/5.^2), 10, 10)
...
if (n_elements(flevels) ge 1) then begin
  ...
  contour,im,cx,cy,xlog=xlog,ylog=ylog,c_colors=color,c_thick=thick,$
    levels=levels,_extra=_extra,xsty=5,ysty=5,/noerase
endif else begin
  contour,im,cx,cy,xlog=xlog,ylog=ylog,_extra=_extra,xsty=5,ysty=5,$
    c_colors=color,c_thick=thick,/noerase
    ; stop
endelse
