...
function density_map, x, y, numbins=numbins, xr=xr, yr=yr, xdata=xdata, ydata=ydata, $
    continuous=continuous, smooth=smooth, normalized=normalized
    ...
    if keyword_set(normalized) then begin
        ids = where(x ge xr[0] and x lt xr[1] and y ge yr[0] and y lt yr[1], cnt)
        factor = float(cnt)
    endif
    ...
    if keyword_set(continuous) then begin
        ...
        if dosmooth then dmap = fltarr(numbins[0], numbins[1]) else dmap = lonarr(numbins[0], numbins[1])
        ...
        if dosmooth then begin
            odmap = fltarr(numbins[0],   numbins[1])
            dmap  = fltarr(numbins[0]*2, numbins[1]*2)
        endif else begin
            dmap  = lonarr(numbins[0]*2, numbins[1]*2)
        endelse
        ...
        for xi=0L, numbins[0]-1 do begin
            ...
            for yi=0L, numbins[1]-1 do begin
                tmp2 = where(yr[0]+yi*ystep le y[tmp1] and y[tmp1] lt yr[0]+(yi+1)*ystep, cnt)
                dmap[2*xi+0, 2*yi+0] = cnt
                dmap[2*xi+1, 2*yi+0] = cnt
                dmap[2*xi+0, 2*yi+1] = cnt
                dmap[2*xi+1, 2*yi+1] = cnt
                if dosmooth then odmap[xi, yi] = cnt
            endfor
        endfor
        ...
        if dosmooth then begin
            for xi=0L, numbins[0]-1 do for yi=0L, numbins[1]-1 do begin
                m = circular_mask(numbins, [xi,yi], smooth)
                idz = where(m ne 0.0)
                cnt = total(odmap[idz]*m[idz])/total(m[idz])
                dmap[2*xi+0, 2*yi+0] = cnt
                dmap[2*xi+1, 2*yi+0] = cnt
                dmap[2*xi+0, 2*yi+1] = cnt
                dmap[2*xi+1, 2*yi+1] = cnt
